...
pub fn parse_asset(text: &str) -> Result<Asset, ReadPDFError> {
    ...
    match cap {
        ...
        Some(cap) => {
            ...
            Ok(Asset{ id: None, name, wkn, isin, note: None })
        }
    }
}
...
pub fn parse_transactions(text: &str) -> Result<(Vec<Transaction>,Asset), ReadPDFError> {
    ...
    lazy_static!{ 
        static ref BUY: Regex = Regex::new(
            r"Wertpapierkauf").unwrap();
        static ref POSITION: Regex = Regex::new(
                r"St.\s*([-0-9.,]*)").unwrap();
        static ref PRE_TAX_AMOUNT: Regex = Regex::new(
                    r"(?m)Zu Ihren Lasten vor Steuern\s*\n.*\s*([0-9.]{10})\s*([A-Z]{3})\s*([-0-9.,]*)").unwrap();
        static ref AFTER_TAX_AMOUNT: Regex = Regex::new(
            r"Zu Ihren Lasten nach Steuern: *([A-Z]{3}) *([-0-9.,]*)").unwrap();
    }
    ...
    if BUY.is_match(text) {
        ...
        let (pre_tax, valuta) = match pre_tax {
            ...
            Some(cap) => {
                ...
                let currency = Currency::from_str(&cap[2])
        ...
        let after_tax = match after_tax {
            None => Err(ReadPDFError::NotFound),
            Some(cap) => {
                let amount = german_string_to_float(&cap[2])?;
                let currency = Currency::from_str(&cap[1])
                    .map_err(|err| ReadPDFError::ParseCurrency(err))?;
        ...
        transactions.push(
            Transaction{
                id: None,
                transaction_type: TransactionType::Asset{ asset_id: 0, position},
                cash_flow: CashFlow{ amount: pre_tax, date: valuta },
                note: None,
            }
        )
    }
