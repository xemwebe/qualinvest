...
fn add_provider(market: &mut finql::Market, token: &Option<String>, source: finql::quote::MarketDataSource) {
    match token {
        Some(token) => {
...
pub fn update_ticker(ticker_id: usize, db: &mut dyn QuoteHandler, config: &Config) -> Result<(), MarketError> {
