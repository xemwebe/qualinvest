...
pub async fn accounts(
    ...
    let user =
        user_opt.ok_or_else(|| Redirect::to(uri!(ServerState::base(), login(Some("accounts")))))?;
        ...
        if let Ok(u_accounts) = db.get_user_accounts(user.userid).await {
            if !u_accounts.iter().any(|a| a.id == Some(account_id)) {
                return Err(Redirect::to(uri!(
                    ServerState::base(),
                    accounts(Some("You can only update your own accounts"))
                )));
            }
        }
        db.update_account(account).await.map_err(|e| {
            Redirect::to(uri!(
                ServerState::base(),
                accounts(Some(format!("Updating account failed: {}", e)))
            ))
        })?;
    } else {
        let account_id = db.insert_account_if_new(account).await.map_err(|e| {
            Redirect::to(uri!(
                ServerState::base(),
                accounts(Some(format!("Adding account failed: {}", e)))
            ))
        })?;
        if !user.is_admin {
            db.add_account_right(user.userid, account_id)
                .await
                .map_err(|e| {
                    Redirect::to(uri!(
                        ServerState::base(),
                        accounts(Some(format!(
                            "Adding user rights to access new account failed: {}",
                            e
                        )))
                    ))
                })?;
...
pub async fn delete_account(
    ...
    if !user.is_admin {
        return Err(Redirect::to(uri!(
            ServerState::base(),
            accounts(Some("You must be admin user to delete accounts!"))
        )));
    }
    ...
    state.postgres_db.delete_account(id).await.map_err(|e| {
        Redirect::to(uri!(
            ServerState::base(),
            accounts(Some(format!("Delete account failed: {}", e)))
        ))
    })?;
    ...
    if let Some(user_id) = new_user.id {
        state
            ...
            .map_err(|e| {
                Redirect::to(uri!(
                    ServerState::base(),
                    accounts(Some(format!("Updating user failed: {}", e)))
                ))
            })?;
        if let Some(password) = &user_form.password {
            state
                ...
                .map_err(|e| {
                    Redirect::to(uri!(
                        ServerState::base(),
                        accounts(Some(format!("Updating user password failed: {}", e)))
                    ))
                })?;
        }
    } else {
        if !user.is_admin {
            return Err(Redirect::to(uri!(
                ServerState::base(),
                accounts(Some("You need to be admin to add a new user"))
            )));
        }
        if let Some(password) = &user_form.password {
            state
                ...
                .map_err(|e| {
                    Redirect::to(uri!(
                        ServerState::base(),
                        accounts(Some(format!("Adding user failed: {}", e)))
                    ))
                })?;
        } else {
            return Err(Redirect::to(uri!(
                ServerState::base(),
                accounts(Some("You must set a password for new users!"))
            )));
        }
    }
...
pub async fn delete_user(
    ...
    if !user.is_admin {
        return Err(Redirect::to(uri!(
            ServerState::base(),
            accounts(Some("You must be admin user to delete users!"))
        )));
    }
    ...
    state.postgres_db.delete_user(id).await.map_err(|e| {
        Redirect::to(uri!(
            ServerState::base(),
            accounts(Some(format!("Delete user failed: {}", e)))
        ))
    })?;
...
pub async fn user_accounts(
    ...
    let old_accounts: HashSet<i32> = state
        .postgres_db
        .get_user_accounts(user_accounts.user_id)
        .await
        .map_err(|e| {
            Redirect::to(uri!(
                ServerState::base(),
                accounts(Some(format!("Updating users accounts user failed: {}", e)))
            ))
        })?
        .into_iter()
        .map(|a| a.id)
    ...
    for u in (&old_accounts - &new_accounts).into_iter() {
        state
            .postgres_db
            .remove_account_right(user_accounts.user_id, u)
            .await
            .map_err(|e| {
                Redirect::to(uri!(
                    ServerState::base(),
    ...
    for u in (&new_accounts - &old_accounts).into_iter() {
        state
            .postgres_db
            .add_account_right(user_accounts.user_id, u)
            .await
            .map_err(|e| {
                Redirect::to(uri!(
                    ServerState::base(),
