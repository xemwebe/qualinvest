...
pub async fn show_quotes(
    ...
    let asset = asset.map_err(|_| {
        Redirect::to(uri!(
            ServerState::base(),
            crate::asset::analyze_asset(Some(asset_id), Some("Invalid asset."))
        ))
    })?;
    let tickers = tickers.map_err(|_| {
        Redirect::to(uri!(
            ServerState::base(),
            crate::asset::analyze_asset(Some(asset_id), Some("Could not get ticker for asset."))
        ))
    })?;
...
pub async fn update_asset_quote(
    asset_id: i32,
    ticker_id: i32,
    user: UserCookie,
    state: &State<ServerState>,
    ...
    if !user.is_admin {
        return Err(Redirect::to(uri!(
            ServerState::base(),
            super::index(Some("You must be admin user to edit quotes!".to_string()))
        )));
    }
    ...
    Ok(Redirect::to(uri!(
        ServerState::base(),
        show_quotes(asset_id, Option::<String>::None)
    )))
}

#[get("/quote/renew_history?<asset_id>&<ticker_id>&<start>&<end>")]
pub async fn renew_history(
    asset_id: i32,
    ticker_id: i32,
    start: String,
    end: String,
    ...
    if !user.is_admin {
        return Err(Redirect::to(uri!(
            ...
            show_quotes(
                asset_id,
                Some("You must be admin user to update quotes!".to_string())
            )
        )));
    }
    ...
    qualinvest_core::update_quote_history(ticker_id, start_date, end_date, db, &state.market_data)
        ...
        .map_err(|_| {
            Redirect::to(uri!(
                ServerState::base(),
                show_quotes(asset_id, Some("Updating quote history failed.".to_string()))
            ))
        })?;
...
pub async fn delete_quote(
    ...
    if !user.is_admin {
        return Err(Redirect::to(uri!(
            ...
            show_quotes(
                asset_id,
                Some("You must be admin user to update quote history!".to_string())
            )
        )));
    }
    db.delete_quote(quote_id).await.map_err(|_| {
        Redirect::to(uri!(
            ServerState::base(),
            show_quotes(asset_id, Some("Deleting of quote failed."))
        ))
    })?;
    Ok(Redirect::to(uri!(
        ServerState::base(),
...
pub async fn new_quote(
    ...
    if !user.is_admin {
        return Err(Redirect::to(uri!(
            ...
            show_quotes(
                asset_id,
                Some("You must be admin user to add new quotes!".to_string())
            )
        )));
    }
    let db = state.postgres_db.clone();
    let asset = db.get_asset_by_id(asset_id).await.map_err(|_| {
        Redirect::to(uri!(
            ServerState::base(),
            show_quotes(asset_id, Some("Failed to get asset!".to_string()))
        ))
    })?;
...
pub async fn add_new_quote(
    ...
    if !user.is_admin {
        return Err(Redirect::to(uri!(error_msg(
            msg = "Admin rights are required to add quotes"
        ))));
    }
    ...
    let _asset = db.get_asset_by_id(quote_form.asset_id).await.map_err(|_| {
        Redirect::to(uri!(
            ServerState::base(),
            show_quotes(quote_form.asset_id, Some("Failed to get asset".to_string()))
        ))
    })?;

    let currency = finql::datatypes::Currency::from_str(&quote_form.currency).map_err(|_| {
        Redirect::to(uri!(
            ServerState::base(),
            show_quotes(quote_form.asset_id, Some("Invalid currency".to_string()))
        ))
    })?;
    ...
    let ticker_id = db.insert_if_new_ticker(&ticker).await.map_err(|_| {
        Redirect::to(uri!(
            ServerState::base(),
            show_quotes(
                quote_form.asset_id,
                Some("Failed to store manual ticker".to_string())
            )
        ))
    })?;
    ...
    db.insert_quote(&quote).await.map_err(|_| {
        Redirect::to(uri!(
            ServerState::base(),
            show_quotes(
                quote_form.asset_id,
                Some("Failed to store quote".to_string())
            )
        ))
    })?;
    ...
    Ok(Redirect::to(uri!(
        ServerState::base(),
        show_quotes(quote_form.asset_id, Option::<String>::None)
    )))
}
